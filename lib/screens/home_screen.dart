...
class _HomeScreenState extends State<HomeScreen> {
      ...
      body: SafeArea(
        child: Row(
          children: [
            ...
            if (_showAIAssist)
              AIAssistPanel(
                onApplyChanges: _applyAIModifications,
  ...
  // Merge AI-proposed YAML updates into local state
  Future<void> _applyAIModifications(
      List<FileModification> modifications) async {
    ...
    setState(() {
      for (final modification in modifications) {
        ...
        final previousContent = _exportedFiles[filePath];

        if (previousContent != null && !_originalFiles.containsKey(filePath)) {
          _originalFiles[filePath] = previousContent;
        }
        ...
        _selectedFilePath = filePath;
        _fileValidationTimestamps[filePath] = DateTime.now();
        _fileUpdateTimestamps[filePath] = DateTime.now();

        _fileControllers.putIfAbsent(
            filePath, () => TextEditingController(text: newContent));
      ...
      _operationMessage =
          'AI proposal merged for ${touchedFiles.length} file(s): ${touchedFiles.join(', ')}';
      _generatedYamlMessage =
          '$_operationMessage\n\nReview changes before syncing to FlutterFlow.';
    });
  }
